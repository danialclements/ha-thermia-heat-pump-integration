...
class ThermiaHeatPump:
    ...
    def operational_status_cooling_status(self) -> Optional[bool]:
        if (
            self.__visible_operational_statuses_map is None
            or "STATUS_COOL" not in self.__visible_operational_statuses_map.values()
        ):
            return None

        return "STATUS_COOL" in self.running_operational_statuses

    @property
    def operational_status_hot_water_status(self) -> Optional[bool]:
        if (
            self.__visible_operational_statuses_map is None
            or "STATUS_HOTWATER" not in self.__visible_operational_statuses_map.values()
        ):
            return None

        return "STATUS_HOTWATER" in self.running_operational_statuses

    @property
    def operational_status_heating_status(self) -> Optional[bool]:
        if (
            self.__visible_operational_statuses_map is None
            or "STATUS_HEAT" not in self.__visible_operational_statuses_map.values()
        ):
            return None

        return "STATUS_HEAT" in self.running_operational_statuses
